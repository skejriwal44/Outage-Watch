...
def model_builder(trainX,classifier_layers,lstm_variant):
...
def model_fit(model,trainX,testX,y_train_classifier,y_test_classifier,epoch_cnt,batch_size): 
    ...
    Args:
        ...
        epoch_cnt: An integer representing number of epochs to run.
        batch_size: An integer representing batch size.
    Returns:
        history: An object representing the history of performance during evaluation.
    """
    history=model.fit(trainX,y_train_classifier,validation_data=(testX,y_test_classifier), epochs=epoch_cnt, batch_size=batch_size,verbose=1)
    return history